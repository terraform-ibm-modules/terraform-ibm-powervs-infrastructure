...
locals {
  ...
  webdispatcher_vsi_exists = local.key_vsi_list_exists ? length([for vsi_name in module.landing_zone.vsi_names : vsi_name if can(regex("${var.prefix}-webdispatcher", vsi_name))]) > 0 : false
  webdispatcher_vsi_ips    = local.webdispatcher_vsi_exists ? [for vsi in module.landing_zone.vsi_list : vsi.ipv4_address if can(regex("${var.prefix}-webdispatcher", vsi.name))] : []
