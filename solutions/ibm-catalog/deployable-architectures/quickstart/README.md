...
| <a name="input_custom_profile"></a> [custom\_profile](#input\_custom\_profile) | To override the t-shirt profile and create PowerVS wokspace instance with custom specifications, specify the sap\_profile\_id or combination of 'cores' and 'memory'. The variable 'sap\_profile\_id' should be set only while creating SAP systems using sap profile ids and must leave the fields 'cores' and 'memory' empty. The 'storage' & 'tier' are optional values. Please set both 'storage' and 'tier' if volumes are to be created. | <pre>object({<br>    sap_profile_id = string<br>    cores          = string<br>    memory         = string<br>    storage        = string<br>    tier           = string<br>  })</pre> | <pre>{<br>  "cores": "",<br>  "memory": "",<br>  "sap_profile_id": null,<br>  "storage": "",<br>  "tier": ""<br>}</pre> | no |
