...
| <a name="input_client_to_site_vpn"></a> [client\_to\_site\_vpn](#input\_client\_to\_site\_vpn) | VPN configuration - the client ip pool, instance id of the secrets manager, VPN server certificate and list VPN client users to access the VPC and PowerVS virtual servers. | <pre>object({<br>    enable                        = bool<br>    client_ip_pool                = string<br>    secrets_manager_id            = string<br>    server_cert_crn               = string<br>    vpn_client_access_group_users = list(string)<br>  })</pre> | <pre>{<br>  "client_ip_pool": "",<br>  "enable": false,<br>  "secrets_manager_id": "",<br>  "server_cert_crn": "",<br>  "vpn_client_access_group_users": []<br>}</pre> | no |
