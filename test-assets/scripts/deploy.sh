...
# SLZ deployment prep
export TEST_LOCATION=$(awk -v key="$PVS_ZONE" -F',' '{ if ($1 == key) { print $2 } }' test-assets/documents/slz_locations.csv)
echo $TEST_LOCATION
...
# 1. Define SLZ Name 
export slzName="auto-test"-"$TEST_LOCATION"-"$(date +'%d-%m-%Y')"
...
export ws_id=`ibmcloud schematics workspace list | grep ${slzName} | awk '{print $2}'`
...
export input_loc=$(pwd)/test-assets/infra-inputs/input.tfvars
echo "input file $input_loc"

# 8. Terraform deploy of powervs infra
echo "STEP 8: PVS Infra deployment"
cd pvs-infra/examples/ibm-catalog/standard-solution
terraform init
